...
import WINDSfunctionsandclasses_Jan29  as wmf
    ...
    if (P.RunPlanting[i] =='TRUE'): #checks to see whether a planting is active and should be analyzed
        ...
        for locate in range(1, int(P.NumLocations[i]) + 1): #Loops through the locations in a planting
            ...
            if P.DateP[i] > dt(2018,1,1) and P.WeatherStationNameP[i] == 'Maricopa':
                ...
                Weather_Array_in = Create_weather_array(Weather2018.loc[(Weather2018['Date'] > P.DateP[i] + timedelta(int(P.First_day - 1))) & (Weather2018['Date'] <= P.DateP[i] + timedelta(int(P.First_day + P.final_day)))],
